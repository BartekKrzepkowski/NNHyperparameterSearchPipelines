...
class TrainerClassification:
    ...
    def run_exp(self, config):
        ...
        for epoch in tqdm(range(config.epoch_start_at, config.epoch_end_at), desc='run_exp'):
            ...
            self.model.eval()
            with torch.no_grad():
                acc = self.run_epoch(phase='test', config=config)
        self.logger.close()
        return acc
    ...
    def run_epoch(self, phase, config):
        ...
        for i, data in enumerate(progress_bar):
            ...
            if whether_epoch_end:
                ...
                if phase == 'test':
                    # print(epoch_assets['evaluators'])
                    acc = epoch_assets['evaluators']['epoch_acc/test'] / epoch_assets['denom']
                    print(acc)
                    config.trial.report(acc, self.epoch)
